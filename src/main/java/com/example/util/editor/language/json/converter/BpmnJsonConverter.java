 ...
 */
package com.example.util.editor.language.json.converter;
...
import com.example.util.editor.constants.EditorJsonConstants;
import com.example.util.editor.constants.StencilConstants;
import com.example.util.editor.language.json.converter.util.CollectionUtils;
import com.example.util.editor.language.json.converter.util.JsonConverterUtil;
import com.example.util.editor.language.json.model.ModelInfo;
...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
  ...
  private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap, BpmnModel bpmnModel) {
    ...
    for (String edgeId : edgeMap.keySet()) {
      ...
      math.geom2d.line.Line2D firstLine = new math.geom2d.line.Line2D(sourceRefLineX, sourceRefLineY, nextPointInLineX, nextPointInLineY);
      ...
      math.geom2d.line.Line2D lastLine = null;
      ...
      if (dockersNode.size() > 2) {
        ...
        lastLine = new math.geom2d.line.Line2D(startLastLineX, startLastLineY, endLastLineX, endLastLineY);
